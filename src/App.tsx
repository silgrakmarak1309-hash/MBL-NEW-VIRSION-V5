...
import { fetchSupabaseListings } from './lib/useListings';
...
export function App() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      // 1. Real Supabase Listings Fetcher (Marketplace filters active, Admin/Seller views see pending too)
      const { data: liveListings, error: listingsErr } = await fetchSupabaseListings({ activeOnly: false });
      if (!listingsErr) {
        setListings(liveListings);
        setListingsError(null);
      } else {
        console.warn('Notice fetching Supabase live listings:', listingsErr);
        setListingsError(listingsErr);
      }
...
setCurrentUser((prev) => {
  ...
  return prev;
    ...
    // 6.5. Local Services & Delivery Registrations
    ...
    try {
      const { data: srvData } = await supabase
        ...
        .select('*')
        .order('created_at', { ascending: false });
      if (srvData) servicesData = [...srvData];
    } catch (_) {}

    try {
      const { data: delRegData } = await supabase
        .from('delivery_registrations')
        .select('*')
        .order('created_at', { ascending: false });
      if (delRegData && delRegData.length > 0) {
        const existingIds = new Set(servicesData.map((s) => String(s.id)));
        for (const dr of delRegData) {
          if (!existingIds.has(String(dr.id))) {
            servicesData.push({
              ...dr,
              category: dr.category || 'Delivery Partner',
              full_name: dr.full_name || dr.name || 'Delivery Partner',
            });
          }
        }
      }
    ...
    let deliveriesData: any[] | null = null;
    try {
    ...
    if (deliveriesData) {
      ...
      // 9. Withdrawal & Payout Requests (All history: pending, completed, rejected)
  ...
  useEffect(() => {
    ...
    checkAndSend3DaysPlanExpiryAlerts().catch((err: any) => {
  ...
  const handleToggleProfileApproval = async (profileOrId: UserProfile | string, approved: boolean) => {
    ...
    if (supabase) {
      ...
      // Synchronize service_registrations table in Supabase
      try {
        const targetProfile = profiles.find((p) => p.id === targetId || (targetEmail && p.email === targetEmail));
        const validUserId = ensureUuid(targetId);

        // Check if a service registration exists for this user_id
        ...
        if (existingSrv && existingSrv.length > 0) {
          ...
          // If approved and no service_registration exists, create one with valid UUID
          const newSrvId = generateUuid();
          await supabase.from('service_registrations').insert([
            {
              id: newSrvId,
              user_id: validUserId,
              full_name: targetProfile?.full_name || targetName,
              phone: targetProfile?.phone || '',
              category: (targetProfile as any)?.shop_category || (targetProfile?.is_delivery_partner ? 'Delivery Partner' : 'Shop Partner'),
              service_type: targetProfile?.is_delivery_partner ? 'driver' : 'shop',
              vehicle_type: targetProfile?.vehicle_type || null,
              vehicle_number: targetProfile?.vehicle_number || null,
              payout_upi: targetProfile?.payout_upi_id || targetProfile?.upi_id || null,
              payout_upi_id: targetProfile?.payout_upi_id || targetProfile?.upi_id || null,
              state: targetProfile?.state || 'Meghalaya',
              district: targetProfile?.district || 'West Garo Hills',
              block: targetProfile?.block || '',
        ...
        console.warn('Supabase service_registrations sync on approval notice:', srvErr);
      }
    }

    // Also update local service_registrations state
    setServiceRegistrations((prev) => {
      const exists = prev.some((s) => s.user_id === targetId || s.user_id === ensureUuid(targetId));
      if (exists) {
        return prev.map((s) =>
          s.user_id === targetId || s.user_id === ensureUuid(targetId)
            ? { ...s, is_approved: approved, status: approved ? 'approved' : 'rejected' }
            : s
        );
      } else if (approved) {
        const targetProfile = profiles.find((p) => p.id === targetId || (targetEmail && p.email === targetEmail));
        const newLocalSrv: ServiceRegistration = {
          id: generateUuid(),
          user_id: ensureUuid(targetId),
          full_name: targetProfile?.full_name || targetName,
          user_name: targetProfile?.full_name || targetName,
          phone: targetProfile?.phone || '',
          category: (targetProfile as any)?.shop_category || (targetProfile?.is_delivery_partner ? 'Delivery Partner' : 'Shop Partner'),
          service_type: targetProfile?.is_delivery_partner ? 'driver' : 'shop',
          vehicle_type: targetProfile?.vehicle_type,
          vehicle_number: targetProfile?.vehicle_number,
          payout_upi: targetProfile?.payout_upi_id || targetProfile?.upi_id,
          payout_upi_id: targetProfile?.payout_upi_id || targetProfile?.upi_id,
          state: targetProfile?.state || 'Meghalaya',
          district: targetProfile?.district || 'West Garo Hills',
          block: targetProfile?.block || '',
          village: targetProfile?.village || '',
          experience: 'Verified Local Partner',
          is_approved: true,
          status: 'approved',
          created_at: new Date().toISOString(),
        };
        return [newLocalSrv, ...prev];
      }
      return prev;
    });
  ...
  const handleSaveSetting = async (key: string, value: string) => {
    ...
    if (supabase) {
      try {
        ...
        dispatchAppToast({
          title: 'Setting Saved',
          message: `${key.replace(/_/g, ' ')} saved successfully.`,
          type: 'success',
        });
      } catch (e: any) {
  ...
  const handleListingSubmitted = (newListing: Listing) => {
    ...
    fetchData(); // Trigger full Supabase re-sync
    setUserActiveTab('marketplace'); // Redirect to live marketplace
  };
  ...
  const handleShopSubmitted = async (newShop: ShopRegistration) => {
    setShopRegistrations((prev) => [newShop, ...prev.filter((s) => s.id !== newShop.id)]);
    if (supabase) {
      try {
        const targetUserId = ensureUuid(currentUser?.id || newShop.user_id);
        const shopPayload: Record<string, any> = {
          user_id: targetUserId,
          user_name: newShop.owner_name || newShop.user_name || currentUser?.full_name || 'Shop Owner',
          user_phone: newShop.user_phone || newShop.phone || currentUser?.phone || '',
          phone: newShop.phone || newShop.user_phone || currentUser?.phone || '',
          shop_name: newShop.shop_name,
          category: newShop.category,
          owner_name: newShop.owner_name,
          shop_address: newShop.shop_address || (newShop as any).address || '',
          address: newShop.shop_address || (newShop as any).address || '',
          city_locality: newShop.city_locality || newShop.village || '',
          state: newShop.state || 'Meghalaya',
          district: newShop.district || '',
          block: newShop.block || '',
          village: newShop.village || '',
          status: newShop.status || 'pending',
          created_at: newShop.created_at || new Date().toISOString(),
        };

        // Note: Do NOT send custom string ID 'shop_...' because Supabase shop_registrations.id is an int8 identity column!
        ...
        if (insertErr) {
          console.warn('shop_registrations full insert notice, retrying with core fields:', insertErr.message);
          const corePayload: Record<string, any> = {
            user_id: targetUserId,
            user_name: shopPayload.user_name,
            shop_name: newShop.shop_name,
            owner_name: newShop.owner_name,
            category: newShop.category,
          ...
          const { data: coreInserted } = await supabase
            .from('shop_registrations')
            .insert([corePayload])
            .select();
          if (coreInserted && coreInserted[0]?.id) {
            setShopRegistrations((prev) =>
              prev.map((s) => (s.id === newShop.id ? { ...s, id: String(coreInserted[0].id) } : s))
            );
          }
        } else if (inserted && inserted[0]?.id) {
          setShopRegistrations((prev) =>
            prev.map((s) => (s.id === newShop.id ? { ...s, id: String(inserted[0].id) } : s))
          );
        }
      } catch (e) {
  ...
  const handleVehicleSubmitted = async (newVeh: VehicleRegistration) => {
    setVehicleRegistrations((prev) => [newVeh, ...prev.filter((v) => v.id !== newVeh.id)]);
    if (supabase) {
      try {
        const targetUserId = ensureUuid(currentUser?.id || newVeh.user_id);
        const vehPayload: Record<string, any> = {
          user_id: targetUserId,
          driver_name: newVeh.driver_name || currentUser?.full_name || 'Driver',
          driver_phone: newVeh.driver_phone || (newVeh as any).phone || currentUser?.phone || '',
          phone: (newVeh as any).phone || newVeh.driver_phone || currentUser?.phone || '',
          vehicle_type: newVeh.vehicle_type,
          vehicle_number: newVeh.vehicle_number || newVeh.vehicle_reg_no || '',
          vehicle_reg_no: newVeh.vehicle_reg_no || newVeh.vehicle_number || '',
          vehicle_model: newVeh.vehicle_model || newVeh.vehicle_type || '',
          driving_license_no: newVeh.driving_license_no || '',
          operational_route: newVeh.operational_route || '',
          state: newVeh.state || 'Meghalaya',
          district: newVeh.district || '',
          block: newVeh.block || '',
          village: newVeh.village || '',
          status: newVeh.status || 'pending',
          created_at: newVeh.created_at || new Date().toISOString(),
        };
        ...
        if (insertErr) {
          console.warn('vehicle_registrations full insert notice, retrying core:', insertErr.message);
          const corePayload: Record<string, any> = {
            user_id: targetUserId,
            driver_name: vehPayload.driver_name,
            driver_phone: vehPayload.driver_phone,
            vehicle_type: newVeh.vehicle_type,
            vehicle_number: vehPayload.vehicle_number,
            status: 'pending',
          };
          const { data: coreInserted } = await supabase
            .from('vehicle_registrations')
          ...
          if (coreInserted && coreInserted[0]?.id) {
            setVehicleRegistrations((prev) =>
              prev.map((v) => (v.id === newVeh.id ? { ...v, id: String(coreInserted[0].id) } : v))
            );
          }
        } else if (inserted && inserted[0]?.id) {
          setVehicleRegistrations((prev) =>
            prev.map((v) => (v.id === newVeh.id ? { ...v, id: String(inserted[0].id) } : v))
          );
        }
      } catch (e) {
  ...
  const handleApproveShopRegistration = async (id: string) => {
    ...
    if (supabase) {
      try {
        const { data: updatedRows, error: updErr1 } = await supabase
          .from('shop_registrations')
          .update({ status: 'approved', verified_at: verifiedTimestamp })
          .eq('id', id)
          .select('id');

        if ((updErr1 || !updatedRows || updatedRows.length === 0) && shop?.user_id) {
          await supabase
            .from('shop_registrations')
            .update({ status: 'approved', verified_at: verifiedTimestamp })
            .eq('user_id', ensureUuid(shop.user_id));
        }
  ...
  const handleApproveVehicleRegistration = async (id: string) => {
    ...
    if (supabase) {
      try {
        ...
        if ((updErr1 || !updatedRows || updatedRows.length === 0) && veh?.user_id) {
          await supabase
            .from('vehicle_registrations')
            .update({ status: 'approved', verified_at: verifiedTimestamp })
            .eq('user_id', ensureUuid(veh.user_id));
        }

        if (veh?.user_id) {
          const validUserId = ensureUuid(veh.user_id);
          try {
            const { error: updErr } = await supabase
              .from('profiles')
              .update({
  ...
  const handleSubmitServiceRegistration = async (
    ...
    if (supabase) {
      try {
        ...
        if (regError) {
          console.warn('service_registrations Supabase notice, retrying without id:', regError.message);
          const srvPayloadNoId: Record<string, any> = {
            user_id: validUserId,
            full_name: data.full_name || data.user_name || 'Service Partner',
            user_name: data.full_name || data.user_name || 'Service Partner',
            phone: data.phone || (data as any).whatsapp || '',
            category: data.category || 'Local Service',
            service_category: data.category || 'Local Service',
            service_type: data.service_type || 'service',
            vehicle_type: (data as any).vehicle_type || null,
            vehicle_number: (data as any).vehicle_number || null,
            driving_license_no: (data as any).driving_license_no || null,
          ...
          const { data: insertedSrv } = await supabase
            .from('service_registrations')
            .insert([srvPayloadNoId])
            .select();
          if (insertedSrv && insertedSrv[0]?.id) {
            setServiceRegistrations((prev) =>
              prev.map((s) => (s.id === validId ? { ...s, id: String(insertedSrv[0].id) } : s))
            );
          }
        } else {
  ...
  const handleApproveServiceRegistration = async (id: string) => {
    ...
    if (supabase) {
      try {
        let updatedCount = 0;
        // 1. Try update by registration id
        ...
        if (!updErr1 && updData && updData.length > 0) {
          updatedCount += updData.length;
        }

        // 2. If not updated by id and user_id is known, try update by user_id
        if (updatedCount === 0 && srv?.user_id) {
          const { data: updUserData, error: updErr2 } = await supabase
            .from('service_registrations')
            .update({ is_approved: true, status: 'approved', verified_at: verifiedTimestamp })
            .eq('user_id', srv.user_id)
            .select('id');

          if (!updErr2 && updUserData && updUserData.length > 0) {
            updatedCount += updUserData.length;
          }
        }

        // 3. If still not in Supabase DB, insert it freshly with valid UUID
        if (updatedCount === 0 && srv) {
          const validRegId = ensureUuid(srv.id);
          const validUserId = ensureUuid(srv.user_id);
          await supabase.from('service_registrations').insert([
            {
              id: validRegId,
              user_id: validUserId,
              full_name: srv.full_name || srv.user_name || 'Partner',
              phone: srv.phone || '',
              category: srv.category || 'Delivery Partner',
              service_type: srv.service_type || 'driver',
              vehicle_type: srv.vehicle_type || null,
              vehicle_number: srv.vehicle_number || null,
              payout_upi: srv.payout_upi || srv.payout_upi_id || null,
              payout_upi_id: srv.payout_upi_id || srv.payout_upi || null,
              state: srv.state || 'Meghalaya',
              district: srv.district || 'West Garo Hills',
              block: srv.block || '',
              village: srv.village || '',
              is_approved: true,
              status: 'approved',
  ...
  const handleRequestPayout = async (payoutData: {
    ...
    if (supabase) {
      try {
        ...
        if (error) {
          console.warn('payout_requests insert with custom id failed, retrying without id:', error.message);
          await supabase.from('payout_requests').insert([payloadData]);
        }
      } catch (err) {
  ...
  const handleRegisterDeliveryPartner = async (data: {
    ...
    try {
      ...
      if (supabase) {
        ...
        if (regError) {
          console.warn('service_registrations Supabase note, retrying without id:', regError.message);
          const noIdSrv = {
            user_id: validUserId,
            full_name: data.fullName,
            phone: data.phone,
          ...
          await supabase.from('service_registrations').insert([noIdSrv]);
        } else {
          fetchAdminRegistrations();
        }

        try {
          await supabase.from('delivery_registrations').insert([
            {
              user_id: validUserId,
              full_name: data.fullName,
              phone: data.phone,
  ...
  const syncDeliveryMutation = async (
    ...
    if (action === 'insert') {
      try {
        const { error: err1 } = await supabase.from('deliveries').insert([payload]);
        if (err1) {
      ...
      try {
        const { error: err1 } = await supabase.from('deliveries').update(payload).eq('id', id);
        if (err1) {
          await supabase.from('delivery_orders').update(payload).eq('id', id);
        }
      } catch (_) {}
    }
  };
  ...
  const handleUpdateDeliveryOrderStatus = async (
    ...
    await syncDeliveryMutation('update', updatePayload, orderId);
  };
  ...
  const handleCancelOrder = async (
    orderId: string,
    reason: string = 'Cancelled by buyer',
    refundAmount: number = 0,
    deliveryChargeRefund: number = 0
  ) => {
  ...
  const handleCreateSampleDeliveryOrder = async (
    ...
    await syncDeliveryMutation('insert', newOrder);
  };
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900 flex flex-col font-sans selection:bg-orange-500 selection:text-white">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8">
        ...
        {currentUser && userActiveTab === 'transactions' && (
          <TransactionLogs
            recharges={rechargeRequests.filter(
              (r) =>
                !r.user_email ||
                r.user_email.toLowerCase() === currentUser.email.toLowerCase() ||
                (currentUser.phone && r.user_phone === currentUser.phone)
            )}
            payouts={payoutRequests.filter(
              (p) =>
                p.user_id === currentUser.id ||
                p.user_id === ensureUuid(currentUser.id) ||
                (currentUser.phone && p.user_phone === currentUser.phone)
            )}
            walletBalance={
              typeof currentUser.wallet_balance === 'number'
                ? Number(currentUser.wallet_balance)
                : (profiles.find((pr) => pr.id === currentUser.id)?.wallet_balance || 0)
            }
        ...
        {currentUser && userActiveTab === 'account' && (
          <AccountSecurity
            ...
            onNavigateToAdmin={() => navigateTo('admin')}
            onUpdateDeliveryPartner={(data) => {
              if (currentUser && typeof data === 'object') {
                handleUpdateDeliveryPartner(
                  currentUser.id,
                  Boolean(data.is_delivery_partner),
            ...
            onUpdatePermanentAddress={async (data) => {
              if (typeof data === 'string') {
                await handleUpdatePermanentAddress(data);
              } else if (data && typeof data === 'object') {
                await handleUpdatePermanentAddress(data.permanent_address || '');
              }
            }}
          />
        )}
      ...
      {/* Partner & Member Payout Withdrawal Modal */}
      {currentUser && (
        <PayoutRequestModal
          ...
          onSubmitPayout={async (pData) => {
            await handleRequestPayout({
              amount: pData.amount,
              upi_id: pData.upi_id || currentUser.payout_upi_id || 'user@upi',
              bank_name: pData.bank_name || currentUser.payout_bank_name,
              account_no: pData.bank_account_no || currentUser.payout_account_no,
              ifsc_code: pData.bank_ifsc || currentUser.payout_ifsc_code,
