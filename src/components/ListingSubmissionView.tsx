...
export const ListingSubmissionView: React.FC<ListingSubmissionViewProps> = ({
  ...
  // Task 1: Geolocation states for seller_latitude and seller_longitude
  ...
  const captureCurrentLocation = React.useCallback(async (): Promise<{ lat: number | null; lng: number | null }> => {
    ...
    return new Promise((resolve) => {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          setSellerLatitude(lat);
          setSellerLongitude(lng);
          setIsLocating(false);
          setLocationStatus('captured');
          resolve({ lat, lng });
        },
        (err) => {
          console.warn('Geolocation capture failed or permission denied:', err?.message || err);
          setIsLocating(false);
          setLocationStatus('failed');
  ...
  // Automatically attempt to capture seller's GPS location on form mount
  useEffect(() => {
  ...
  const handleSubmitListing = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Resolve valid seller_id referencing profiles.id to prevent FK constraint violations
      let resolvedSellerId: string | null = null;
      if (supabase) {
        try {
          const { data: authData } = await supabase.auth.getUser();
          if (authData?.user?.id) {
            resolvedSellerId = authData.user.id;
          }
        } catch (_) {}

        if (!resolvedSellerId && currentUser?.email) {
          try {
            const { data: prof } = await supabase
              ...
              .maybeSingle();
            if (prof?.id) {
              resolvedSellerId = prof.id;
            }
          } catch (_) {}
        }

        if (!resolvedSellerId && userId) {
          try {
            const { data: prof } = await supabase
              .from('profiles')
              .select('id')
      ...
      // Task 1: Ensure GPS coordinates are captured before submission
      let currentLat = sellerLatitude;
      let currentLng = sellerLongitude;
      ...
      // Pack metadata into description for cross-schema reliability
      const metaObj = {
        price: parsedPrice,
        category,
        condition,
        ...
        seller_name: userName || 'Verified Local Seller',
        seller_latitude: currentLat !== null ? Number(currentLat) : null,
        seller_longitude: currentLng !== null ? Number(currentLng) : null,
        weight: safeNumericWeight,
        is_heavy_item: isHeavyItem,
        is_featured: Boolean(isProUser),
        is_pro: Boolean(isProUser),
        status: 'pending', // Task 2: Default to pending for admin moderation
        created_at: new Date().toISOString(),
      };
      const cleanUserDesc = (description || '').trim();
      const metaComment = `<!--MLB_META:${JSON.stringify(metaObj)}-->`;
      const finalDbDescription = cleanUserDesc ? `${cleanUserDesc}\n\n${metaComment}` : metaComment;

      // Full payload attempting all columns if available in remote database
      const fullDbPayload: Record<string, any> = {
        id: finalListingId,
        title: trimmedTitle,
        description: finalDbDescription,
        price: parsedPrice,
        ...
        seller_latitude: currentLat !== null ? Number(currentLat) : null,
        seller_longitude: currentLng !== null ? Number(currentLng) : null,
        delivery_fee: parsedPrice,
        image_urls: finalImageUrls,
        state: locationState.state || 'Meghalaya',
        district: locationState.district || null,
        block: locationState.block || null,
        village: locationState.village || null,
      ...
      // Guaranteed schema payload matching exact verified columns in listings table
      const fallbackCleanPayload: Record<string, any> = {
        id: finalListingId,
        title: trimmedTitle,
        description: finalDbDescription,
        status: 'pending',
      ...
      if (!supabase) {
        throw new Error('Supabase client is not available. Please check your internet connection.');
      }

      let insertError: any = null;
      let insertedRow: any = null;

      // Execute real INSERT into Supabase listings table
      const primaryRes = await supabase.from('listings').insert([fullDbPayload]).select();
      if (primaryRes.error) {
        console.warn('Primary insert notice, retrying with schema-adapted payload:', primaryRes.error.message);
        // Column mismatch (e.g. price or status not in listings schema yet)
        if (
          primaryRes.error.code === 'PGRST204' ||
          primaryRes.error.code === '42703' ||
          primaryRes.error.message?.includes('schema cache') ||
          primaryRes.error.message?.includes('does not exist')
        ) {
          const retryRes = await supabase.from('listings').insert([fallbackCleanPayload]).select();
          if (retryRes.error) {
            // Foreign key failure on seller_id
            if (retryRes.error.code === '23503' || retryRes.error.message?.includes('foreign key')) {
              console.warn('Foreign key notice on seller_id, retrying without seller_id');
              const { seller_id, ...noSellerPayload } = fallbackCleanPayload;
              const fkRetry = await supabase.from('listings').insert([noSellerPayload]).select();
              insertError = fkRetry.error;
              insertedRow = fkRetry.data?.[0];
            } else {
        ...
        } else if (primaryRes.error.code === '23503' || primaryRes.error.message?.includes('foreign key')) {
          console.warn('Foreign key notice on seller_id, retrying without invalid seller_id');
          const { seller_id, ...noSellerPayload } = fallbackCleanPayload;
          const fkRetry = await supabase.from('listings').insert([noSellerPayload]).select();
          insertError = fkRetry.error;
          insertedRow = fkRetry.data?.[0];
        ...
        insertedRow = primaryRes.data?.[0];
      }

      if (insertError) {
        throw new Error(
          `Supabase failed to insert listing: ${insertError.message || 'Database rejected insertion.'}`
        );
      }

      // Success notification toast (Task 2: Pending Approval)
      dispatchAppToast({
        title: 'Listing Submitted for Review! ⏳',
        message: `"${trimmedTitle}" is pending admin approval and will go live once verified.`,
        type: 'success',
      });

      const listingPayload: Listing = {
        id: insertedRow?.id || finalListingId,
        title: trimmedTitle,
        category_name: category,
        ...
        is_pro: Boolean(isProUser),
        status: 'pending',
        seller_latitude: currentLat !== null ? Number(currentLat) : undefined,
        seller_longitude: currentLng !== null ? Number(currentLng) : undefined,
        seller_id: resolvedSellerId || undefined,
        seller_name: userName || 'Verified Local Seller',
    ...
    <div className="max-w-2xl mx-auto bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden relative">
      ...
      <form onSubmit={handleSubmitListing} className="p-6 sm:p-8 space-y-5">
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3">
          ...
          {/* Task 1: GPS Coordinates Feedback Box */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-3 bg-white rounded-xl border border-slate-200 text-xs">
            <div className="flex items-center gap-2">
              ...
              {sellerLatitude !== null && sellerLongitude !== null ? (
                <span className="font-mono font-bold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-md border border-emerald-200 flex items-center gap-1">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                  {sellerLatitude.toFixed(4)}° N, {sellerLongitude.toFixed(4)}° E
                </span>
              ) : isLocating ? (
                <span className="text-slate-500 flex items-center gap-1">
                  <Loader2 className="w-3.5 h-3.5 animate-spin text-orange-600" />
                  Detecting GPS via browser...
                </span>
              ) : (
                <span className="text-amber-700 bg-amber-50 px-2 py-0.5 rounded-md border border-amber-200 text-[11px] font-medium">
                  Auto-detecting on submission...
                </span>
              )}
            </div>
            ...
            <button
              ...
              disabled={isLocating}
              className="px-3 py-1 rounded-lg text-xs font-bold text-orange-600 hover:text-orange-700 hover:bg-orange-50 border border-orange-200 transition cursor-pointer self-start sm:self-auto disabled:opacity-50"
            >
              {isLocating ? 'Detecting...' : sellerLatitude ? 'Refresh GPS' : 'Detect GPS'}
            </button>
          </div>
        </div>
