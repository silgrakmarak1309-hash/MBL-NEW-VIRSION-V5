...
export const UserMarketplace: React.FC<UserMarketplaceProps> = ({
  ...
  // Strictly show only active approved listings (Task 2 admin approval requirement)
  // and hide listings from sellers whose subscription plan has expired (Task 3)
  const activeListings = listings.filter((item) => {
    // 1. Task 2: Listing status must be 'active' (not 'pending', 'rejected', or 'inactive')
    const isVisible = item.status === 'active';
    if (!isVisible) return false;

    // 2. Task 3: Check if the seller's subscription plan is expired
    if (profiles && profiles.length > 0 && item.seller_id) {
      const seller = profiles.find((p) => p.id === item.seller_id || (item.phone && p.phone === item.phone));
      if (seller) {
        // If seller account has an expired subscription, hide listing from marketplace
        if (!isUserPlanActive(seller)) {
          return false;
        }
      }
    }

    // Also check if listing itself has an expiry date
    if ((item as any).seller_plan_expiry) {
      const expiry = new Date((item as any).seller_plan_expiry).getTime();
        ...
        <div className="flex items-center justify-between">
          <h3 className="text-base sm:text-lg font-black text-slate-900 flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-slate-500">
              ({loading ? 'Loading...' : `${activeListings.length} active`})
            </span>
          </h3>
          {onRefresh && (
            <button
              ...
              className="p-1.5 text-slate-500 hover:text-orange-600 hover:bg-slate-100 rounded-lg transition text-xs flex items-center gap-1 cursor-pointer"
            >
        ...
        {loading ? (
          <div className="bg-white rounded-2xl p-8 sm:p-12 text-center border border-slate-200 shadow-xs space-y-3">
            ...
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              Connecting to Supabase database to fetch real-time community products.
            </p>
          </div>
        ) : error ? (
          /* Error State with Retry Button */
          <div className="bg-red-50 rounded-2xl p-6 sm:p-8 text-center border border-red-200 shadow-xs space-y-3">
            <AlertTriangle className="w-8 h-8 text-red-600 mx-auto" />
            <h4 className="text-sm sm:text-base font-bold text-red-800">
              Unable to load listings from database
            </h4>
            ...
            {onRefresh && (
              <button
                type="button"
                onClick={onRefresh}
                className="mt-2 px-4 py-2 bg-red-600 text-white rounded-xl text-xs font-bold hover:bg-red-700 transition inline-flex items-center gap-2 cursor-pointer shadow-xs"
              >
          ...
          <div className="bg-white rounded-2xl p-8 sm:p-12 text-center border border-slate-200 shadow-xs">
            ...
            <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
              {searchQuery || selectedCategory !== 'All'
                ...
                : 'No active listings currently in the database. Be the first to post a product or service!'}
            </p>
            <div className="flex items-center justify-center gap-2 mt-4">
              {(searchQuery || selectedCategory !== 'All') && (
                <button
                  ...
                  className="px-3.5 py-2 bg-slate-100 text-slate-700 rounded-xl text-xs font-bold hover:bg-slate-200 transition cursor-pointer"
                >
                  Clear Filters
                </button>
              )}
              <button
                ...
                className="px-4 py-2 bg-orange-600 text-white rounded-xl text-xs font-bold hover:bg-orange-700 transition inline-flex items-center gap-1.5 cursor-pointer shadow-xs"
              >
