...
export type Setting = AdminSetting;
...
export function isUserPlanActive(user?: UserProfile | null): boolean {
  ...
  if ((user as any).account_status === 'suspended' || (user as any).account_status === 'banned') {
    return false;
  }

  // Check expiration: if plan_expiry_date is in the past, subscription has expired
  ...
  if ((user as any).pro_expired_at) {
    const expiryTime = new Date((user as any).pro_expired_at).getTime();
    if (!isNaN(expiryTime) && expiryTime < Date.now()) {
      return false;
    }
  }
  ...
  // 4. Future valid plan_expiry_date check
  if (user.plan_expiry_date) {
    const expiryTime = new Date(user.plan_expiry_date).getTime();
    if (!isNaN(expiryTime) && expiryTime > Date.now()) {
      return true;
